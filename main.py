...
current_weather_cache = TTLCache(maxsize=100, ttl=300)
...
async def get_current_weather(location: str) -> dict[str, Any]:
    ...
    latitude, longitude, resolved_location = await get_coordinates(location)
...
@app.get("/current")
async def current_weather(location: str = "Guntur") -> dict[str, Any]:
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail=f"Unable to retrieve current weather: {error}",
        ) from error
...
async def chat(request: ChatRequest) -> ChatResponse:
    if chat_service is None:
        raise HTTPException(
            status_code=503,
            detail=(
                "WeatherGPT AI service could not be initialized. "
    ...
    try:
        answer = chat_service.process(question)
...
async def cache_status() -> dict[str, Any]:
    return {
        ...
        "weather_cache_size": len(current_weather_cache),
